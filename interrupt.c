...
rvu_word (*trap_handlers[24]) (RiscvGPRS *, rvu_word);
...
TrapCause trap_read_cause(rvu_word scause_reg) {
    bool is_interrupt = bits(scause_reg, XLEN-1, 1);
    uint32_t trap_code = bits(scause_reg, 0, XLEN-1);
    if (is_interrupt) {
...
void supervisor_handle_trap(RiscvGPRS * regs) {
    ...
    rvu_word new_sepc = (*trap_handlers[cause])(regs, sepc);
    sepc_w_csr(new_sepc);
}
